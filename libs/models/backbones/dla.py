...
class DLA(nn.Module):
    ...
    def load_pretrained_model(self, data='imagenet', name='dla34', hash='ba72cf86'):
        ...
        if name.endswith('.pth'):
            model_weights = torch.load(data + name, map_location='cpu')
            self.load_state_dict(model_weights, strict=False)
            return

        file_name = f'{name}-{hash}.pth'
        
        possible_paths = [
            os.path.join(os.getcwd(), 'pretrained', file_name), # Local execution
            os.path.join('/work/pretrained', file_name),        # Hardcoded Docker path
        ]

        local_path = next((path for path in possible_paths if os.path.exists(path)), None)
        ...
        try:
            ...
            if local_path:
                ...
                raise FileNotFoundError(
                    f"Could not download weights AND no local file found in any expected directories."
                )
